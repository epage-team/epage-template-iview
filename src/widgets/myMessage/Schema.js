...
export default class MyMessageSchema extends Epage.Schema {
  constructor (schema, widgets, descriptor) {
    super() // super 不传参数时  $init 不会执行
    this.title = ''
    this.option.message = '' // 消息内容
    this.option.type = 'tip' // 消息类型
    this.$init(schema, widgets, descriptor) // 初始化schema、widget、descriptor
  }
}
